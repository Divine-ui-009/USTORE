...
// Create a dedicated axios instance that always reads the token fresh
const authAxios = axios.create({ baseURL: API });

// Interceptor runs before every request — reads token from localStorage each time
...
// Interceptor on responses — surfaces the real server error message
...
// Admin only — go through authAxios which attaches the token automatically
...
export const getProductsPaged = (page = 0, size = 9, sortBy = "id", dir = "asc") =>
  axios.get(`${API}/paged?page=${page}&size=${size}&sortBy=${sortBy}&dir=${dir}`);